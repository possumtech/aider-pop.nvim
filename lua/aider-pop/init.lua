...
function M.send(text)
	...
	local prefix = map[text:sub(1, 1)] or ""
	local msg = prefix ~= "" and text:sub(2):gsub("^%s+", "") or text
	if prefix == "/" then prefix, msg = "", text:gsub("^/%s+", "/") end
...
function M.status()
	...
	return (M.buffer and vim.api.nvim_buf_line_count(M.buffer) > M.last_read_line) and "🤖" or ""
end
